window.onload = function () {
  ...
  function editarForm(e) {
    ...
    document.getElementById("visitor-name").value =
      json_scoreboard.visitor_team;
    document.getElementById("visitor-points").value =
      json_scoreboard.visitor_points;
  ...
  function getTextTrackJson() {
    ...
    for (let index = 0; index < cues.length; index++) {
      ...
      text +=
        ...
        '"' +
        ', "time":"' +
        convertTimeToVttFormat(cues[index].startTime) +
        " --> " +
        convertTimeToVttFormat(cues[index].endTime) +
        '"' +
        ', "text":' +
        cues[index].text +
        "}";
  ...
  function convertTimeToVttFormat(time) {
    ...
    return (
      twoDigits(hour) + ":" + twoDigits(min) + ":" + twoDigits(sec) + ".000"
    );
  }
